...
\b
    Examples
        vcd network isolated create "isolated-net1" \\
             --gateway-ip 192.168.1.1 --netmask 255.255.255.0 \\
            --description "Isolated VDC network" \\
            ...
            Creates an isolated org vdc network with an inbuilt dhcp service.
    """
    if ctx.invoked_subcommand is not None:
        try:
            restore_session(ctx)
